...
export const verifyPayment = async (req: Request, res: Response) => {
  try {
    ...
    if (existingTransaction?.type === 'TICKET_PURCHASE' || metadata?.type === 'ticket_purchase') {
      const raffleId = metadata?.raffleId as string | undefined;
      const targetUserId = (metadata?.userId as string | undefined) || req.user!.userId;
    ...
    sendPushToUser(req.user!.userId, {
      ...
      body: `Your wallet deposit of ₦${amountInNaira.toLocaleString()} was successful.`,
      url: '/dashboard/wallet',
    }).catch((err) => console.error('[Payment] Deposit push error:', err));
