...
export const getDashboardStats = async (_req: Request, res: Response) => {
  try {
    ...
    const [
      ...
      // Total revenue: sum of completed ticket purchases
...
export const getAnalytics = async (req: Request, res: Response) => {
  try {
    ...
    const [
      ...
      prisma.transaction.aggregate({
        where: { type: 'TICKET_PURCHASE', status: 'COMPLETED', createdAt: { gte: rangeStart } },
        _sum: { amount: true },
      }),
      prisma.transaction.aggregate({
        where: {
      ...
      prisma.user.count({
        where: { role: 'USER', createdAt: { gte: prevRangeStart, lt: rangeStart } },
      }),
    ...
    const dailyMap: Record<
      string,
      { revenue: number; tickets: number; users: number; deposits: number }
    > = {};
    ...
    const ticketsChart = Object.entries(dailyMap).map(([date, v]) => ({
      date,
      tickets: v.tickets,
    }));
    ...
    res.status(200).json({
      ...
      data: {
        ...
        revenueBreakdown: {
          monnifyDeposits: realDeposits._sum.amount || 0,
          walletBonuses: bonusRewards._sum.amount || 0,
          ticketSales: ticketPurchases._sum.amount || 0,
        },
        charts: {
  ...
  try {
    ...
    const task = await prisma.task.create({
      data: {
        ...
        adDuration: adDuration ? Number.parseInt(adDuration) : null,
        maxCompletions: maxCompletions ? Number.parseInt(maxCompletions) : null,
        dailyLimit: dailyLimit ? Number.parseInt(dailyLimit) : null,
        expiresAt: expiresAt ? new Date(expiresAt) : null,
        priority: priority !== undefined ? Number.parseInt(priority) : 0,
  ...
  try {
    ...
    const task = await prisma.task.update({
      ...
      data: {
        ...
        ...(points !== undefined && { points: Number.parseInt(points) }),
        ...
        ...(priority !== undefined && { priority: Number.parseInt(priority) }),
...
export const convertPrizeToWallet = async (req: Request, res: Response) => {
  try {
    ...
    if (raffle.deliveryStatus === 'CONVERTED_TO_WALLET') {
      res.status(400).json({
        success: false,
        message: 'This prize has already been converted to store credits.',
      });
      return;
    }
    ...
    const [updatedUser, updatedRaffle] = await prisma.$transaction([
      ...
      prisma.raffle.update({
        where: { id: raffleId },
        data: {
          deliveryStatus: 'CONVERTED_TO_WALLET',
          deliveryNote:
            note ||
            `Converted item value (₦${creditAmount.toLocaleString()}) to non-withdrawable wallet credits.`,
          deliveryUpdatedAt: new Date(),
        },
      }),
    ]);
...
export const extendRaffleTimer = async (req: Request, res: Response) => {
  try {
    ...
    if (!newRaffleDate) {
      res
        .status(400)
        .json({ success: false, message: 'Please provide a valid new expiration date & time.' });
      return;
    }
    ...
    if (Number.isNaN(targetDate.getTime()) || targetDate <= new Date()) {
      res
        .status(400)
        .json({ success: false, message: 'New expiration date must be in the future.' });
      return;
    }
...
// POST /api/admin/announcement - Send broadcast notification to all users
export const sendAnnouncement = async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: 'Announcement broadcast push notification sent to all active subscriptions.',
    });
  } catch (error) {
    console.error('[Admin] Send announcement push error:', error);
    res
      .status(500)
      .json({ success: false, message: 'Failed to send announcement push notification.' });
  }
};
