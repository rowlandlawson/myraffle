...
export const getUserTickets = async (req: Request, res: Response) => {
  try {
    ...
    const where: { userId: string; status?: string } = { userId: req.user!.userId };
...
export const buyTicket = async (req: Request, res: Response) => {
  try {
    ...
    const qty = Math.max(1, Math.min(10, Number.parseInt(quantity) || 1));
    ...
    if (user.role === 'ADMIN') {
      res.status(403).json({
        success: false,
        message:
          'Administrators are strictly prohibited from participating in raffles or purchasing tickets.',
      });
      return;
    }
    ...
    if (paymentMethod === 'wallet') {
      ...
      try {
        ...
        if (message === 'RAFFLE_SOLD_OUT') {
          res.status(400).json({
            success: false,
            message:
              'Sorry! The remaining ticket(s) for this raffle were just purchased by another user. This raffle is now sold out.',
          });
          return;
        }
        if (message === 'INSUFFICIENT_WALLET_FUNDS') {
          res.status(400).json({
            success: false,
    ...
    try {
      ...
      const errMessage = monnifyErr instanceof Error ? monnifyErr.message : 'Unknown payment error';
      console.error('[Monnify Error]:', errMessage);
      res.status(400).json({
        success: false,
        message: `Monnify Payment Initialization Error: ${errMessage}`,
      });
      return;
    }
