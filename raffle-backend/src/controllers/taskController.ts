  ...
  try {
    ...
    if (task.type === 'REFERRAL') {
      res.status(400).json({
        success: false,
        message:
          'Referral rewards are awarded automatically when your referred friend verifies their email.',
      });
      return;
    }
    ...
    if (task.type.startsWith('SOCIAL_') || task.type.startsWith('WATCH_AD_')) {
      ...
      if (!startedAt) {
        res.status(400).json({
          success: false,
          message:
            'Task verification failed. Please ensure you open the task and complete it before claiming.',
        });
        return;
      }
      ...
      if (Number.isNaN(startTime) || elapsed < requiredDurationMs) {
        // Stealth response: neutral failure message without revealing return time
        res.status(400).json({
          success: false,
          message:
            'Task verification failed. Please make sure you completed the task requirement on social media and try again.',
        });
        return;
      }
    }
    ...
    if (pointsToAward <= 0) {
      res
        .status(400)
        .json({ success: false, message: 'This task has no point reward configured.' });
      return;
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (task.type.startsWith('SOCIAL_')) {
        ...
        if (alreadyCompleted) {
          return {
            error: 'You have already completed this social task. Rewards can only be claimed once.',
          };
        }
      }
      ...
      if (task.dailyLimit && task.dailyLimit > 0) {
        ...
        if (completionsToday >= task.dailyLimit) {
          return {
            error: `You've reached the daily limit of ${task.dailyLimit} for this task. Check back tomorrow!`,
          };
        }
      }
...
export const getCompletedTasks = async (req: Request, res: Response) => {
  try {
    ...
    const where: { userId: string; completedAt?: { gte: Date } } = { userId };
    if (rangeStart) {
