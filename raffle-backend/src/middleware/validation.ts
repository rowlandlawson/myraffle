...
export const depositSchema = z.object({
  amount: z.number({ message: 'Amount must be a number' }).min(100, 'Minimum deposit is ₦100'),
});

export const withdrawalSchema = z.object({
  amount: z.number({ message: 'Amount must be a number' }).min(500, 'Minimum withdrawal is ₦500'),
