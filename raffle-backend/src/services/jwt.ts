...
export const generateAccessToken = (userId: string, role: string): string => {
  return jwt.sign({ userId, role } as TokenPayload, env.JWT_SECRET, {
    expiresIn: CONSTANTS.JWT_ACCESS_EXPIRY,
  });
};

export const generateRefreshToken = (userId: string, role: string): string => {
  return jwt.sign({ userId, role } as TokenPayload, env.JWT_REFRESH_SECRET, {
...
export const generateTempToken = (userId: string, method: string): string => {
  return jwt.sign({ userId, method } as TempTokenPayload, env.JWT_SECRET, { expiresIn: '5m' });
};
