...
export const runRaffleDraw = async (raffleId: string) => {
  ...
  sendRaffleWinnerEmail(winner.email, winner.name, raffle.item.name, raffle.item.value).catch(
    (err: Error) => {
  ...
  sendPushToUser(winner.id, {
    ...
    body: `Congratulations ${winner.name}! You won the draw for ${raffle.item.name}!`,
    url: '/dashboard/wins',
  }).catch((err) => console.error('[Raffle] Failed to send winner push notification:', err));
...
export const checkAndTriggerAutoDraw = async (raffleId: string) => {
  try {
    ...
    if (raffle.ticketsSold >= raffle.ticketsTotal) {
      console.log(
        `[AutoDraw] Raffle ${raffleId} is completely sold out (${raffle.ticketsSold}/${raffle.ticketsTotal}). Executing winner draw immediately!`,
      );
      const drawResult = await runRaffleDraw(raffleId);
      console.log(
        `[AutoDraw] Winner selected for raffle ${raffleId}: User ${drawResult.winnerName} (${drawResult.winnerUserNumber})`,
      );
    }
  } catch (error) {
