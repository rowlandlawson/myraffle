...
export function useRaffles(params?: {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let rafflesList: ApiRaffle[] = [];
      if (
        rawData &&
        typeof rawData === 'object' &&
        'raffles' in rawData &&
        Array.isArray((rawData as { raffles: ApiRaffle[] }).raffles)
      ) {
        rafflesList = (rawData as { raffles: ApiRaffle[] }).raffles;
      } else if (Array.isArray(rawData)) {
        rafflesList = rawData as ApiRaffle[];
      } else if ('raffles' in res && Array.isArray((res as Record<string, unknown>).raffles)) {
        rafflesList = (res as { raffles: ApiRaffle[] }).raffles;
      }

      const pagination =
        rawData && typeof rawData === 'object' && 'pagination' in rawData
          ? (rawData as { pagination: Pagination }).pagination
          : null;
