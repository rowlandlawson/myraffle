import { Award, CheckCircle, Clock, ExternalLink, Play, Share2 } from 'lucide-react';
...
export default function TaskCard({
  ...
  const _isAdTask = task.type.startsWith('watch_ad');
  ...
  return (
    <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-5 hover:shadow-md transition flex flex-col h-full relative overflow-hidden">
      ...
      <div
        className={`absolute top-0 left-0 right-0 h-1 opacity-50 ${getBadgeColors().split(' ')[1].replace('text-', 'bg-')}`}
      />

      <div className="flex items-start justify-between mb-3 mt-1">
        <div
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border ${getBadgeColors()}`}
        >
          {getTypeIcon()}
        ...
        {task.platform && (
          <>
            ...
            <div className="flex flex-col flex-1">
              ...
              <span className="text-gray-700 text-sm font-semibold capitalize truncate">
                {task.platform}
              </span>
            </div>
          </>
        ...
        {dailyLimit != null && dailyLimit > 0 && (
          <>
            ...
            <div className="flex flex-col flex-1">
              ...
              <span
                className={`text-sm font-semibold ${limitReached ? 'text-red-500' : 'text-gray-700'}`}
              >
      ...
      <button
        ...
        className={`w-full py-2.5 rounded-xl font-bold transition flex items-center justify-center gap-2 ${
          isCompleted || limitReached
            ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
            : isSocialTask && isVerifying
              ? 'bg-gray-100 text-gray-500 cursor-wait'
              : isSocialTask && canClaim
                ? 'bg-green-600 text-white hover:bg-green-700 shadow-sm shadow-green-200'
                : isClickedUrl
                  ? 'bg-green-600 text-white hover:bg-green-700 shadow-sm shadow-green-200'
                  : task.actionUrl && !isClickedUrl && !isSocialTask
