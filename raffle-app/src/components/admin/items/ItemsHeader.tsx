...
export default function ItemsHeader({ showUploadForm, onToggleUploadForm }: ItemsHeaderProps) {
  return (
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Raffle Items</h1>
        <p className="text-gray-600">Manage all raffle items and their settings</p>
      </div>
      <button
      ...
      >
        <Plus size={20} /> {showUploadForm ? 'Cancel Upload' : 'Upload New Item'}
      </button>
    </div>
  );
}
