  ...
  if (!transactions || transactions.length === 0) {
    return (
      <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div className="text-sm text-gray-500 text-center py-8">No recent transactions found</div>
      </div>
    );
  }
          ...
          return (
            ...
            >
              ...
              <div className="text-right">
                <p className={`text-sm font-bold ${style.color}`}>₦{tx.amount.toLocaleString()}</p>
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${getStatusBadge(tx.status)}`}
                >
                  {tx.status}
                </span>
              </div>
