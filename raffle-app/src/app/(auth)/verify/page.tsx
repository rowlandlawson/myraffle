...
function VerifyPageContent() {
  ...
  const handleResendPhone = async () => {
    ...
    try {
      const endpoint =
        phoneChannel === 'sms' ? '/api/auth/resend-sms-otp' : '/api/auth/resend-whatsapp-otp';
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Email Verification */}
          <div
            className={`bg-white rounded-2xl shadow-sm border p-6 transition-all ${emailVerified ? 'border-green-300 bg-green-50/30' : 'border-gray-200'}`}
          >
            <div className="flex items-center gap-3 mb-4">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center ${emailVerified ? 'bg-green-100' : 'bg-red-100'}`}
              >
                {emailVerified ? (
          ...
          <div
            className={`bg-white rounded-2xl shadow-sm border p-6 transition-all ${phoneVerified ? 'border-green-300 bg-green-50/30' : 'border-gray-200'}`}
          >
            <div className="flex items-center gap-3 mb-4">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center ${phoneVerified ? 'bg-green-100' : 'bg-blue-50'}`}
              >
                {phoneVerified ? (
                  <CheckCircle2 className="text-green-600" size={20} />
                ) : (
        ...
        <p className="text-center text-xs text-gray-400 mt-8">
          Codes expire in 10 minutes. Didn&apos;t receive a code? Try switching between WhatsApp and
          SMS above.
        </p>
      </div>
    </div>
